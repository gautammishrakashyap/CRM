...
from app.core.dependencies import (
    get_user_dep, get_users_dep, delete_user_dep, update_user_dep, 
    create_user_dep, authenticate_user, get_user_id_dep
)
from app.core.authorization import (
    get_current_user, requires_admin, requires_read_users, 
    requires_write_users, requires_delete_users
)
from app.schema.user import ListUsersResponse, UpdateUserResponse, User, Token
...
def create_user(user_created: User = Depends(create_user_dep)):
...
async def login_for_access_token(
    ...
    """
    login_id = form_data.username or form_data.email
    if not login_id:
        raise HTTPException(
    ...
    # 🧠 Authenticate
    user = authenticate_user(username=login_id, password=form_data.password)
    if not user:
        raise HTTPException(
...
@router.get(
    ...
    description="Requires Bearer token and read_users permission",
    dependencies=[Depends(bearer_scheme)],
    tags=['USER V1']
)
def get_user_by_id(
...
@router.delete(
    ...
    response_model_exclude_none=True,
    summary="Delete user",
    description="Requires Bearer token and delete_users permission",
    dependencies=[Depends(bearer_scheme)],
    tags=['USER V1']
)
...
@router.patch(
    ...
    response_model_exclude_none=True,
    summary="Update user",
    description="Requires Bearer token and write_users permission",
    dependencies=[Depends(bearer_scheme)],
    tags=['USER V1']
)
def update_user(
